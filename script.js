...
async function showBubble(amount, icon) {
  ...
  for (let Index = 0; Index < numHearts; Index++) {
    ...
    setTimeout(() => {
      // Thêm lớp explode cho hiệu ứng chính
      bubble.classList.add("explode");

      // Tạo 5 mini-icon nổ tại chỗ
      let randomCategory = "🌹";
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (bubbleIsGrabbed) {
          miniIcon.innerHTML = randomCategory;
        } else {
          miniIcon.innerHTML = icon;
        }
        bubble.appendChild(miniIcon);
      }

      bubble.style.visibility = "hidden";
      // Xóa bubble sau khi hiệu ứng hoàn thành
      setTimeout(() => {
        bubble.remove();
      }, 600); // Thời gian hiệu ứng explode
    }, 5000);
